...
setwd("~/Documents/TISCH/GSEA_cluster_all/")
outpath <- "../GSEA_cluster_all_figures/"
  ...
  gsea_res <- lapply(gsea_dir, function(ff){
    ...
    tryCatch({
      ...
      file.tmp <- subset(file, select = c("NAME","NES","FDR.q.val")) %>% mutate(direction = dirction) %>%
        mutate(cluster = cl) %>% #mutate(cell = cluster_cell[cluster,"cell_level2"]) %>%
...
split_gsea_direction <- function(gsea_res){
  ...
  for (i in c("pos","neg")){
    ...
    gsea_res_mat <- reshape2::dcast(gsea_sub, NAME~cluster, value.var = "logFDR")
...
###function of generating complexheatmap for each matrix
PlotGseaHeatmap <- function(mat, direction, cluster_cell, title, ysize = 7.3){
  ###top anntation for cell grouping in heatmap body
  # add cell level2 label
  cell_level2 <- as.character(cluster_cell[colnames(mat),"cell_level2"])
  ...
  ht_list = Heatmap(mat.new, name = "-log10FDR", # expression(-log[10]("FDR"))
                    ...
                    heatmap_height = unit(nrow(mat.new)*0.5+5, "cm"),
  ...
  ###add line for separating cell types
...
PlotGSEA <- function(folder, type, outpath){
  ...
  cc_rows <- paste("cluster_", rownames(cluster_cell), sep = "") 
  ...
  for (gsea_direction in c("UP","DOWN")){
    ...
    p <- PlotGseaHeatmap(mat = gsea_mat, direction = gsea_direction, cluster_cell = cluster_cell, title = title)
    print(p)
    dev.off()
  }
}
